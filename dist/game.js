...
var _Spritesheet = __webpack_require__(335);
...
var Map = function () {
    ...
    _createClass(Map, [{
        ...
        value: function render(ctx) {
            ...
            for (var y = 0; y < 16; ++y) {
                for (var x = 0; x < 16; ++x) {
                    if (this._data[y][x] != 0) tileset.render(ctx, x * this.TILE_SIZE, y * this.TILE_SIZE, this._data[y][x]);
                }
            }
        }
    }, {
        key: 'getCollisions',
        value: function getCollisions(pos) {
            var left = false;
            var right = false;
            var up = false;
            var down = false;
            if (this._data[pos.y][pos.x - 1] === undefined || this.COLLISIONS_TILE.includes(this._data[pos.y][pos.x - 1])) left = true;
            if (this._data[pos.y][pos.x + 1] === undefined || this.COLLISIONS_TILE.includes(this._data[pos.y][pos.x + 1])) right = true;
            if (this._data[pos.y + 1] === undefined || this._data[pos.y + 1][pos.x] === undefined || this.COLLISIONS_TILE.includes(this._data[pos.y + 1][pos.x])) down = true;
            if (this._data[pos.y - 1] === undefined || this._data[pos.y - 1][pos.x] === undefined || this.COLLISIONS_TILE.includes(this._data[pos.y - 1][pos.x])) up = true;
    ...
    return new Promise(function (resolve, reject) {
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState === xhr.DONE) {
                if (xhr.status === 200) resolve(JSON.parse(xhr.responseText));else reject("Can't load json named " + url);
            }
        };
        xhr.send(null);
    });
...
var _Level = __webpack_require__(334);
...
var Game = function () {
    ...
    _createClass(Game, [{
        ...
        value: function run() {
            ...
            this.init();
            _Loader2.default.load().then(function () {
                _this.load();
...
var Spritesheet = function () {
    ...
    _createClass(Spritesheet, [{
        ...
        value: function render(ctx, x, y, type) {
            ...
            ctx.drawImage(this._image, sx, sy, this._tilesize, this._tilesize, x, y, this._tilesize, this._tilesize);
        }
    }]);
...
var _Vector = __webpack_require__(337);
...
var Player = function () {
    function Player(pos, mapName) {
        ...
        this._vector = new _Vector2.default();
        this._move = {
            state: -1,
            frame: 0,
            direction: 0, // 0: down, 1: left, 2: up, 3: right
            offset: new _Position2.default()
        };
    ...
    _createClass(Player, [{
        key: 'setNextMove',
        value: function setNextMove(type, orientation) {
            var map = _Loader2.default.getMap(this._mapName);

            var collision = map.getCollisions(this._pos);
        ...
        value: function update(delta) {
            if (this._move.state === -1) {
                if (_Keyboard2.default.isPress("ArrowUp")) this.setNextMove("up", 2);else if (_Keyboard2.default.isPress("ArrowDown")) this.setNextMove("down", 0);else if (_Keyboard2.default.isPress("ArrowRight")) this.setNextMove("right", 3);else if (_Keyboard2.default.isPress("ArrowLeft")) this.setNextMove("left", 1);
            }
            this.updateMove();
        }
    }, {
        ...
        value: function updateMove() {
            ...
            if (this._move.state >= this.ANIMATION_TIME) {
                ...
                if (this._move.frame > 2) this._move.frame %= 3;

                var pixelToMove = 64 - 64 * (this._move.state / this.ANIMATION_TIME);

                this._move.offset.x = 0;
                this._move.offset.y = 0;
                if (this._move.direction == 0) {
                    // DOWN
                    this._move.offset.y = -pixelToMove;
                } else if (this._move.direction == 1) {
                    // LEFT
                    this._move.offset.x = pixelToMove;
                } else if (this._move.direction == 2) {
                    // UP
                    this._move.offset.y = pixelToMove;
                } else if (this._move.direction == 3) {
                    // RIGHT
                    this._move.offset.x = -pixelToMove;
                }
